"""Shared utilities for axon-variation simulations."""
...
def count_assignment_axons(axon_assignment_file: Path | str) -> int:
    """Number of axons (non-empty lines) in an assignment file."""
    n_axons = 0
    with open(axon_assignment_file, "r", encoding="utf-8") as handle:
        for line in handle:
            if line.strip():
                n_axons += 1
    return n_axons
...
def load_axon_topology(
    ...
    *,
    T_ms: float = 0.0,
    seed: int = 0,
    n_axons: int | None = None,
) -> tuple[list[int], list[list[int]]]:
    """Load axon-to-synapse assignment without generating events.

    ``n_axons`` defaults to the number of non-empty lines in the file.
    ``T_ms`` and ``seed`` are unused and kept for call-site compatibility.
    """
    del T_ms, seed
    if n_axons is None:
