"""
TS1 with multi-synapse axon inputs
"""
...
# Configuration
REPORT_NAME = "TS1_axons" + "_" + datetime.now().strftime("%Y%m%d_%H%M%S")
REPORT_FILEPATH = get_simulation_path("ts1", "axons", "results", f"{REPORT_NAME}.pdf")
...
PULSE_TIME_MS = 50.0
N_AXONS = 10
SEQUENTIAL_AXON_ORDER: List[int] = [5, 6, 9, 8 , 2 , 4, 7, 1, 3 ,0]
SYNC_X_MAX_MS = 80.0
SYNC_X_STEP_MS = 20.0
SYNC_N_TRIALS = 10
SYNC_RNG_SEED = 0
...
results_path = get_simulation_path("ts1", "axons_results")
...
def _add_sequential_axon_pages(
    ...
    report.add_paragraph(
        f"Ten stitched {SHORT_T_MS:.0f} ms runs in order {SEQUENTIAL_AXON_ORDER}. "
...
def main():
    ...
    report.add_title("TS1 Axons Study (Short Simulations)")
