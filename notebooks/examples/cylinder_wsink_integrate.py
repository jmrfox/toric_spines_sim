...
# %% [markdown]
# Canonical example: cylinder + sink integration.
#
# Goal: run ``TSSimulator`` on ``cylinder_wsink_r20um.swc`` and inspect sink voltage.
# Open via Jupyter (`uv run jupyter lab`) from the repo root.
...
# %% [markdown]
# # Integration study for cylinder model
#
# First, we set up a function to do the simulation for a given set of input rates and parameters.

# %%
swc_filepath = get_swc_path("cylinder_wsink_r20um.swc", units="microns")
synpts_filepath = get_pointset_path("cylinder_synpts.txt", units="microns")
total_synapses = 3
model_name = "cylinder"
...
input_rates_hz[0] = input_rates_hz[1] = 40.0
...
plotter_ts = TimeSeriesPlotter(
    title="Membrane potential in sink", xlim=(0, 500.0), figsize=(12, 4)
)
plotter_ts.add_time_series_from_arbor(results["probe"])
plotter_ts.show()
plotter_rp = RasterPlotter(
    title="Input event streams", xlim=(0, 500.0), figsize=(12, 4)
)
...
plotter.add_time_series(time, v_1[0] - 70, label="V(1)", color="tab:blue", row=0)
plotter.add_time_series(time, v_2[0] - 70, label="V(2)", color="tab:blue", row=1)
plotter.add_time_series(
    time, v_1[0] + v_2[0] - 70, label="V(1)+V(2)", color="tab:blue", row=2
)
plotter.add_time_series(time, v_12[0] - 70, label="V(1+2)", color="tab:blue", row=3)
...
# %% [markdown]
# ## Make K matrix plots for many pairs of synapses
...
n_pairs = 3
synapse_pairs = random.sample(all_synapse_pairs, n_pairs)
rate_list = [25, 50, 75, 100, 125]

compute_all_k_matrices = False  # WARNING: This may take a long time!
...
if compute_all_k_matrices:
    for active_synapse_pair in synapse_pairs:
        ...
        savefile = f"../out/ts1_kmatrix_s{active_synapse_pair[0]}_s{active_synapse_pair[1]}.png"
...
if make_report:
    ...
    report = PdfReport("../out/cylinder_integration_report.pdf")
    report.add_title("TS2 with sink integration")
    report.add_paragraph(
        f"""This report aggregates multiple simulation runs with their parameters, metrics, and figures.
        TS2 has {total_synapses} synapses.
