...
plotter_ts.add_time_series_from_arbor(
    results["probe"],
    linewidth=1.0,
)
plotter_ts.show()
plotter_rp = RasterPlotter(title="Input event streams", xlim=(0, 1000))
...
if compute_all_k_matrices:
    for active_synapse_pair in tqdm(synapse_pairs):
        results = compute_k_matrix(
            total_synapses=total_synapses,
            active_synapse_pair=active_synapse_pair,
            rate_list=rate_list,
...
with open(f"../out/{model_name}_k_matrices_by_synapse_pair.pkl", "wb") as f:
